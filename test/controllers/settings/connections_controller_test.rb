...
class Settings::ConnectionsControllerTest < ActionDispatch::IntegrationTest
  ...
  test "disconnecting leaves another user's connection alone" do
    ...
    assert_difference "Connection.count", -1 do
      delete settings_connections_url
    end

    assert_nil @user.reload.connection
    assert_not_nil @other.reload.connection
  end
  ...
  test "create opens a grant and shows the waiting state" do
    login_as @user
    DeviceFlow.any_instance.stubs(:start).returns(grant)
  ...
  test "create says so when the app cannot be reached" do
    login_as @user
    DeviceFlow.any_instance.stubs(:start).returns(nil)
  ...
  test "poll reports idle with no grant in flight" do
    login_as @user
    get poll_settings_connections_url
  ...
  test "poll reports pending while waiting" do
    login_as @user
    DeviceFlow.any_instance.stubs(:start).returns(grant)
    post settings_connections_url

    DeviceFlow.any_instance.stubs(:check).returns([ :pending, nil ])
    get poll_settings_connections_url
  ...
  test "poll keeps waiting when the app is briefly unreachable" do
    login_as @user
    DeviceFlow.any_instance.stubs(:start).returns(grant)
    post settings_connections_url

    DeviceFlow.any_instance.stubs(:check).returns([ :unreachable, nil ])
    get poll_settings_connections_url
  ...
  test "poll stores the connection against the user who approved it" do
    ...
    DeviceFlow.any_instance.stubs(:check).returns([
    ...
    connection = @user.reload.connection
  ...
  test "reconnecting replaces your row rather than piling up" do
    ...
    DeviceFlow.any_instance.stubs(:start).returns(grant)
    post settings_connections_url, params: { base_url: "https://links.example.com" }
    DeviceFlow.any_instance.stubs(:check).returns([ :approved, { "token" => "new" } ])

    get poll_settings_connections_url

    assert_equal 1, @user.reload.connection ? 1 : 0
    assert_equal "new", @user.reload.connection.token
  end
  ...
  test "poll reports a denial and forgets the grant" do
    ...
    DeviceFlow.any_instance.stubs(:check).returns([ :denied, nil ])
    get poll_settings_connections_url
