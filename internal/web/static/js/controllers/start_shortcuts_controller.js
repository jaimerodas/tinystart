...
//
// Only the value a page can act on is set — show gets edit, edit gets view.
// So "does nothing on the page it takes you to" comes out of the markup,
...
export default class extends Controller {
  ...
  // key a chord produces says nothing about which key the user pressed.
  isChord(event, code) {
    return event.altKey && !event.metaKey && !event.ctrlKey && event.code === code
  }

  // The command bar is autofocused, so ? must be typeable there. It works as
  // a shortcut only when focus is not inside an input, textarea, select, or
  // editable element.
  isHelp(event) {
  ...
  leaveFor(event, url) {
    ...
    // Leaving is leaving, whichever way you do it. A row that is still
    // carried in the editor must be dropped and saved before the page goes.
    // That is the same rule Tab and clicking away already follow.
    // grid_keyboard listens for this rather than being called, so the two
    // stay strangers.
    //
    // It answers with the save it started. Waiting on that is the whole
    // point. Sending the POST first does not mean the server processes it
    // first, and the destination page reads the same table. Landing before
    // the move commits renders the order it was about to replace.
  ...
  // panel inline instead of in the top layer. There is no backdrop, no focus
  // trap, and Esc no longer reaches it. openDialog also refuses to reopen
  // something it already believes is open. Close it before it can be
  // photographed.
  closeDialog() {
